-- A full-text arm that tolerates questions.
--
-- websearch_to_tsquery ANDs every term, so the text arm only ever fired for
-- keyword-style queries. "how does the nightly reconcile job work" becomes
-- 'night' & 'reconcil' & 'job' & 'work', which no chunk satisfies, while
-- "nightly reconcile" matches 22. A question is how this store is actually
-- asked, so in practice hybrid search was vector search.
--
-- The text arm now also takes chunks that match the OR of the terms, under two
-- guards measured on the golden set (ingest/eval/golden.yaml, 2026-09-21):
--
--   1. at least half the query's distinct lexemes, and never fewer than two.
--      Off-topic questions matched 1-2 of 4-7 lexemes ("best", "change"); the
--      chunks worth having matched 3/4 and 5/6.
--   2. cosine similarity within 0.08 of min_similarity (0.62 at the default
--      floor). Keyword-matched chunks for off-topic questions peaked at 0.58;
--      the relevant ones that the 0.70 floor was discarding sat at 0.67-0.69.
--      With no embedding or no floor there is nothing to compare, and guard 1
--      stands alone.
--
-- A chunk that satisfies the strict AND query is admitted exactly as before,
-- with no similarity requirement. Ranking inside the arm uses the OR query, so
-- ts_rank_cd rewards a chunk for every term it covers and for their proximity.
--
-- The OR form is derived from websearch_to_tsquery's own output rather than by
-- parsing the user's text, which keeps the property that arbitrary punctuation
-- can never raise. It is skipped when the query negates a term (OR-ing a
-- negation matches nearly everything) or has a single lexeme. Matched lexemes
-- are counted by array membership, not by building a tsquery per lexeme, for the
-- same reason: a lexeme such as a Windows path carries a backslash.
--
-- Signature, return type and every filter are unchanged, so no client changes.
...
begin
  ...
  txt as (
    select c.id as cid,
           row_number() over (order by ts_rank_cd(c.tsv, coalesce(loose_query, strict_query)) desc) as rnk
    from rag.chunks c
    join rag.documents d on d.id = c.document_id
    where strict_query is not null
      ...
      and (filter_metadata   is null or d.metadata  @> filter_metadata)
      and (keep_superseded or coalesce(d.metadata ->> 'status', '') <> 'superseded')
    order by ts_rank_cd(c.tsv, coalesce(loose_query, strict_query)) desc
    limit greatest(match_count * 10, 100)
  ),
  fused as (
