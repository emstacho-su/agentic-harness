...
- **Machine file.** `~/.harness/machine.env` says what this machine is:
  ...
  HARNESS_REALMS=projects:push,classes:local       # realms this machine may hold, and whether each may leave it
...
3. In each of `~/vault/projects` and `~/vault/classes`: `echo <name> > .realm`,
   `git init -b main`, a `.gitignore` with `.obsidian/workspace*`, `git add -A`,
   commit, `git remote add origin <private GitHub repo>`, `git push -u origin main`.
4. Write `~/.harness/machine.env` with `HARNESS_VAULT=C:/Users/<you>/vault`,
   `HARNESS_REALMS=projects:push,classes:local`, `HARNESS_MACHINE=home-pc`.
